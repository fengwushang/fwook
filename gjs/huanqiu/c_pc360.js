...
(function(p) {
    var p = p;
    var e = p.using("$baseName.Utility");
    var l = p.using("$baseName.Business");
    var b = "c";
    var g = "inlay";
    var m = "SOGOU_STAR_SETJSONADSLOT";
    l.randomArray.push(Math.random() * 1000000);
    var k = function() {
        if (top == self) {
            return 0
        } else {
    ...
    var d = function(r) {
        ...
        var s = 2;
        if (q[q.length - 2] == "com" || q[q.length - 2] == "net" || q[q.length - 2] == "org") {
            s += 1
        }
        q.splice(0, q.length - s);
        return q.join(".")
    };
    var j = function() {
        if (k() == 1) {
            ...
            var q = r(document.referrer);
            if (q.hostname.lastIndexOf(d(document.location.hostname)) == -1) {
                return 1
            } else {
                if (q.hostname != document.location.hostname) {
                    return 2
                }
            }
        }
    ...
    var h = window[m] = function(C, s, r) {
        ...
        x.set("id", C);
        var v = x.get("bi");
        var q = '<div style="display:none">-</div> <iframe id="{iframeId}" onload="document.sogou_ads{bi}_loaded=true;" src="{starServiceUrl}?{paramString}" width="{iframeWidth}" height="{iframeHeight}" align="center,center" marginwidth="0"  marginheight="0" scrolling="no" frameborder="0" allowtransparency="true" ></iframe>';
        var B = l.getAdsDomId(x.get("bi"));
        var u = l.Param.serialize(x);
        u += "&tmp_cdif=" + String(j());
        var t = {
            iframeId: B,
            bi: v,
            ...
            iframeWidth: y,
            iframeHeight: w,
            starServiceUrl: l.starServiceUrl
        };
        var A = e.g("star_" + x.get("id"));
        if (A) {
            A.innerHTML = e.template(q, t)
        } else {
            document.write(e.template(q, t))
        }
        var z = l.Param.snap(x);
        l.adsArray.push({
            domId: B,
            uiParamSnap: z,
            win: window,
        ...
        l.addAdsCounter(g, window);
        x.set("bi");
        l.ViewWatch.getInstance();
        if (typeof(document.wb_pb) == "undefined") {
            document.wb_pb = new Array()
        }
        window.setTimeout("(function(){if(!document.sogou_ads" + v + '_loaded){var a=new Image();a.src = "http://wb.brand.sogou.com/ct_pv?asid=' + C + "&r=" + x.get("rnd") + '";document.wb_pb.push(a);}}())', 6000)
    };
    if (b === "c") {
        ...
        var o = f[f.length - 1].src;
        var n = e.getPara(o);
        l.isAsyn = n.asyn;
        if (window.sogou_ad_id) {
            var i = window.sogou_ad_id;
            var c = window.sogou_ad_width;
            var a = window.sogou_ad_height;
            window[m](i, c, a)
        }
        window.sogou_ad_id = null
    }
})(window[___sogouNamespaceName]);
