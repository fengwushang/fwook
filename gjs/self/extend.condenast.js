...
var $_c_d = [];
function $_c_v(d,v){$_c_d[d]=v};
function $_c_g(d,v){eval("window."+d+"='"+v+"'")};
function $_c_u(d,g){g=='global'?eval("delete window."+d+""):delete $_c_d[d]};
function $_c_r(a,b){if(!b) var b = location.href;var rs = new RegExp('[\&|\?]{1}'+a+'=([\\w]+)').exec(b);if(rs == null) return null;return rs[1];};
function $_c_j(s){if(!$_c_d['root']){document.write('not root is defined!');$_c_v('root','no');return}document.write('<script type="text/javascript" language="javascript" src="' +$_c_d['root']+ 'src/js/'+s + '.js"></script>');};
function $_c_c(s){if(!$_c_d['root']){document.write('not root is defined!');$_c_v('root','no');return}document.write('<LINK href="' + $_c_d['root']+'src/css/' +s + '.css" type="text/css" rel="stylesheet">');};
function $_c_i(s){if(!$_c_d['root']){document.write('not root is defined!');$_c_v('root','no');return}return $_c_d['root']+'src/img/' +s;};
function $_c_f(s){if(!$_c_d['root']){document.write('not root is defined!');$_c_v('root','no');return}return $_c_d['root']+'src/flash/' +s;};
function $_c_e(e){return $_c_d[e]?eval($_c_d[e]):eval(e)};
function $_c_w(e){$_c_d[e]?$.alert($_c_d[e]):$.alert(e)};
...
/*
$_c_v('win','$_c_w(4)');//注册到全局数组中
//$_c_e('win');
//$_c_w('root');
var a = $_c_e('e','中国');
var b = $_c_e('null',"");
var c = $_c_e('len',"2f分你好");
var d = $_c_e('os');
var e = $_c_e('int',11);
$_c_e('debug',funcc);
$_c_e('load',funcc);
alert($_c_e('time',86400));
*/
...
//弹出窗口
function w(i,u,t,n,g)
{
	$.msg('<form class="postForm" rel="'+i+'" action="'+u+'" method="post" send="ajax">'+t+'</form>',n+'页面',n,function(){$.form(i,function(r){if(r<0){$.alert(''+n+'失败',function(){w(i,u,t,n,g)});return;}else{g.call()}},function(){w(i,u,t,n,g)})});
}
//窗口重载
function load(url)
{
...
(function($)
{
	...
	$.extend({
		...
		form : function(rel,func,pre)
		{
			...
			var data = {};
			$(".postForm[rel="+rel+"] .post").each(function(i){if($(this).attr('id')){eval('data.'+$(this).attr('id')+'='+$_c_e('e',$(this).val()));return $(this).check()}})
	...
	$.fn.iframeSubmit = function (ob)
	{
		...
		if(p.cb) 
		{
            ...
            {
			    $(this).prepend('<input type="hidden" name="function" id="function" value="'+temp[1]+'" style="display:none;"/>');
            }
		}
		if(!p.debug)
		{
	...
	{
		if(options == 'close') return $('.overlay, .box').hide().remove();//非手工关闭方式暂不支持关闭回调
		//参数默认值
		var p = $.extend({
			width: 600,
			height: 0,					//设为0则自动调整高度,不出现滚动条
			overlay: true,				//是否使用覆盖阴影
			border:'#666666',				//整体的边框颜色
			background: 'url("http://www.vogue.com.cn/application/views/front/images/vogue-loginbg1.png") no-repeat',
			title: '',					//如果为空,不显示标题栏
			...
			buttonClass: 'login-rbtn',		//按钮的class
			style: '',					//主容器的css设置
			titleStyle:'',				//标题栏css设置
			titleImg:1					//标题栏背景设置
		}, options || {});
		...
		//创建覆盖层
		if(p.overlay) $.overlay({opacity:0.6});
		...
		//box的调整
		box.css({position:'absolute',visibility:'hidden',overflow:'hidden',width:p.width,zIndex:9999,fontloadsize:'12px',color:'#666',
			background: p.background,border:'0px solid '+p.border+'',padding: '20px 6px 10px 6px',overflowY:p.overflow});
        ...
        */
		if(p.closeBtn) 
		{
			box.prepend('<a class="boxClose" href="javascript:;" title="点击或按esc键关闭"><img src="'+$_c_i('close/'+p.closeImg)+'.gif"/></a>');
		...
		if(p.title) 
		{
			...
			if(p.titleImg)
			{
				titlePos.backgroundImage = 'url("http://www.vogue.com.cn/application/views/front/imageslogin-close.jpg")';
		...
		if(p.buttons.length) 
		{
			//box.append('<div class="boxBtn login-btnbox" style="margin-top: 16px;text-align:'+p.buttonAlign+';"></div>');
            box.append('<div class="boxBtn" style="text-align:'+p.buttonAlign+';"></div>');
			
		}
		for(k in p.buttons) 
		{
			box.find('.boxBtn').append('<button style="margin:0 -5px;margin-top:-10px;margin-left:-10px;float:none;display:inline;">'+p.buttons[k].text+'</button>')
		...
		var epilogue = function ()
		{
			...
			$("#boxContent").css({'margin-top':'10px'});
		}
		...
		if(p.str) 
		{
			box.prepend('<div id="boxContent" style="height:75px;margin:10px;"><div style="margin:10px;font-size:14px;">'+p.str+'</div></div>');
			epilogue();
			p.show.call();
		}
		else if(p.url) 
	...
	$.fn.extend({
		...
		overlay : function (options)
		{
            ...
            o.width(p.W).height(p.H).css('opacity',m.o).css('background','#000').show();
		...
		loadsize : function ()
		{
			...
			var H = document.documentElement.scrollHeight;;//height
			$_c_v('w',W);
			$_c_v('h',H);
